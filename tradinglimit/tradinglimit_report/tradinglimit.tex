...
Sebelum nasabah melakukan transaksi margin,
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
Meskipun demikian, sistem Profindo menggunakan parameter untuk menghitung trading limit sebagai berikut
\begin{figure}[H]
    \centering
...
\begin{equation}
    \text{Haircut KPEI}>65\%.
\end{equation}
Akibatnya, saham ADMR masuk ke dalam kelas saham D/E/F dan tidak bisa diperjualbelikan nasabah.
\\ \\
...
\begin{equation}
    TL_X=MC_X\times\text{Cash}+EBR_X\times\sum_{k=1}^{n}{\left(MS_k\times\min(\text{Lot}_k\times100\times\text{Price}_k\times(1-\text{Haircut}_k),\text{Capping}_k)\right)}
\end{equation}
dengan
\begin{align*}
